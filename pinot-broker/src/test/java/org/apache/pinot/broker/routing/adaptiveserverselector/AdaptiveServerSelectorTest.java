...
public class AdaptiveServerSelectorTest {
  ...
  public void testAdaptiveServerSelectorFactory() {
    ...
    _properties.put(CommonConstants.Broker.AdaptiveServerSelector.CONFIG_OF_ENABLE_STATS_COLLETION, true);
  ...
  public void testNumInFlightReqSelector() {
    ...
    for (int ii = 0; ii < 10; ii++) {
      for (String server : _servers) {
        ++taskCount;
        serverRoutingStatsManager.recordStatsForQuerySubmit(-1, server);
      }
    }
    ...
    for (int ii = 0; ii < _servers.size(); ii++) {
      for (int jj = 0; jj < ii; jj++) {
        ++taskCount;
        serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(ii));
      }
    }
    waitForStatsUpdate(serverRoutingStatsManager, taskCount);
    ...
    serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(0));
    serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(0));
    serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(2));
    serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(2));
    serverRoutingStatsManager.recordStatsForQuerySubmit(-1, _servers.get(2));
    waitForStatsUpdate(serverRoutingStatsManager, taskCount);
    ...
    for (int ii = 0; ii < 1000; ii++) {
      ...
      ++taskCount;
      serverRoutingStatsManager.recordStatsForQuerySubmit(-1, selectedServer);
      waitForStatsUpdate(serverRoutingStatsManager, taskCount);
      ...
      if (rand.nextBoolean()) {
        ++taskCount;
        serverRoutingStatsManager.recordStatsForQueryCompletion(-1, selectedServer, 1);
  ...
  public void testLatencySelector() {
    ...
    _properties.put(CommonConstants.Broker.AdaptiveServerSelector.CONFIG_OF_WARMUP_COUNT, warmupCount);
    ...
    for (int ii = 0; ii < 10; ii++) {
      for (String server : _servers) {
        latencyMap.computeIfAbsent(server,
            k -> new ExponentialMovingAverage(alpha, autodecayWindowMs, warmupCount, avgInitializationVal));
        latencyMap.get(server).compute(2);

        ++taskCount;
        serverRoutingStatsManager.recordStatsForQueryCompletion(-1, server, 2);
      }
    }
    waitForStatsUpdate(serverRoutingStatsManager, taskCount);
    ...
    for (int ii = 0; ii < _servers.size(); ii++) {
      ...
      for (int jj = 0; jj < 10; jj++) {
        ...
        ++taskCount;
        serverRoutingStatsManager.recordStatsForQueryCompletion(-1, server, latency);
      }
    }
    waitForStatsUpdate(serverRoutingStatsManager, taskCount);
    ...
    for (int ii = 0; ii < 1000; ii++) {
      ...
      selectedServer = serverRankingWithVal.get(0).getLeft();
      ++taskCount;
      serverRoutingStatsManager.recordStatsForQueryCompletion(-1, selectedServer, latencyMs);
  ...
  public void testHybridSelector() {
    ...
    _properties.put(CommonConstants.Broker.AdaptiveServerSelector.CONFIG_OF_AVG_INITIALIZATION_VAL,
        avgInitializationVal);
    _properties.put(CommonConstants.Broker.AdaptiveServerSelector.CONFIG_OF_HYBRID_SELECTOR_EXPONENT,
    ...
    // Test 4: Populate servers with unequal latencies and numInFlightRequests.
    for (int ii = 0; ii < _servers.size(); ii++) {
      String server = _servers.get(ii);
      int latency = ii * 2 + 1;
