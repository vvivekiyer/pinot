...
public class AdaptiveServerSelectorTest {
  ...
  public void testNumInFlightReqSelector()
    ...
    for (int ii = 0; ii < 1000; ii++) {
      ...
      _serverRoutingStatsManager.recordStatsForQuerySubmit(-1, selectedServer);

      if (rand.nextBoolean()) {
        int latency = rand.nextInt(1000);
        _serverRoutingStatsManager.recordStatsForQueryCompletion(-1, selectedServer, latency);
      }
    }
    ...
    for (int ii = 0; ii < 1000; ii++) {
      ...
      List<String> serverRanking = new ArrayList<>();
  ...
  public void testMinLatencySelector()
    ...
    for (int ii = 0; ii < 1000; ii++) {
      ...
      if (serverRanking.size() == 0) {
        ...
        double prevVal = 0.0;
        for (String server : serverRanking) {
          double newVal = latencyMap.get(server).getAverage();
          assertTrue(newVal >= prevVal, newVal + " " + prevVal + " " + server);
          prevVal = newVal;
        }
      }
      ...
      latencyMap.computeIfAbsent(selectedServer, k -> new ExponentialMovingAverage(30 * 1000, 30 * 1000, 0));
