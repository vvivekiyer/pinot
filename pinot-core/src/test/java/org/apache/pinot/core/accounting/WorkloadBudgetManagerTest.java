...
public class WorkloadBudgetManagerTest {
  ...
  void testAddOrUpdateAndRetrieveBudget() {
    ...
    assertEquals(1_000_000L, stats.cpuRemaining);
    assertEquals(1_000_000L, stats.memoryRemaining);
  }
  ...
  void testTryChargeWithoutBudget() {
    ...
    assertEquals(Long.MAX_VALUE, stats.cpuRemaining);
    assertEquals(Long.MAX_VALUE, stats.memoryRemaining);
  }
  ...
  void testBudgetResetAfterInterval() throws InterruptedException {
    ...
    assertEquals(500_000L, usedStats.cpuRemaining);
    assertEquals(500_000L, usedStats.memoryRemaining);
    ...
    assertEquals(1_000_000L, resetStats.cpuRemaining);
    assertEquals(1_000_000L, resetStats.memoryRemaining);
  }
  ...
  void testConcurrentTryChargeSingleWorkload() throws InterruptedException {
    ...
    assertEquals(initialCpuBudget - totalCpuCharged, remaining.cpuRemaining, "CPU budget mismatch after concurrent updates");
    assertEquals(initialMemBudget - totalMemCharged, remaining.memoryRemaining, "Memory budget mismatch after concurrent updates");
  }
}
